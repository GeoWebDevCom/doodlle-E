...
<xsl:variable name="subcatsNumber">
	...
	<xsl:template match="/category">
		...
		<div class="SPListing">
            ...
            <xsl:if test="count (categories/category)">
                <input id="SPCatsShow" class="inputbox" name="SPCatsShow" value="{$ExOptLabel}" type="button"/>
            </xsl:if>
				...
				<xsl:for-each select="categories/category">
					<div class="spCatListCell" style="width: {$cellWidth}%">
						<xsl:choose>
							...
							<xsl:otherwise><div class="spclear" /></xsl:otherwise>
						</xsl:choose>
